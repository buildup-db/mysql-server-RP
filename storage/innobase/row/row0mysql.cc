                              ...
                              multi_value & blob data */
{
  ...
  if (UNIV_UNLIKELY(prebuilt->table->fts)) {
...
static dberr_t row_insert_for_mysql_using_ins_graph(const byte *mysql_rec,
  ...
  if (UNIV_UNLIKELY(table->is_corrupted())) {
...
run_again:
  ...
  if (UNIV_UNLIKELY(node->is_delete) ||
      UNIV_UNLIKELY(!(node->cmpl_info & UPD_NODE_NO_ORD_CHANGE))) {
    ...
    dict_table_t *handler)      /*!< in/out: table handler. */
{
  ...
  */
  ...
  if (err == DB_SUCCESS && UNIV_UNLIKELY(is_fts)) {
    dict_index_t *idx;
...
dberr_t row_drop_table_for_mysql(const char *name, trx_t *trx, bool nonatomic,
  ...
  if (!table->is_temporary() && UNIV_LIKELY(!table->is_fts_aux())) {
